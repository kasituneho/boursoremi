...
$requete_dividende=mysql_query("select action.nom,dividende.titre,dividende.annee,dividende.valeur from action,dividende where annee >= \"$annee_debut\" and annee < \"$annee\" and action.titre=dividende.titre and action.active=1 order by action.nom,annee asc;")OR die('Erreur de la requête MySQL');
...
while($row = mysql_fetch_array($requete_dividende)) 
{
	...
	$DIVIDENDE=round($row["valeur"],3);
	echo '<td bgcolor="#CCCCCC">'.$DIVIDENDE.'</td>';
